...
class CollectTruncateSpec extends FunSpec with Matchers with DataFrameSuiteBase
  ...
  describe("collect_truncate tests") {
    it("check result cardinality") {
      ...
      val limited = df.
        groupBy($"loyal").
        agg(
          CollectLimit.collect_truncate(struct($"id", $"name"), 3).as("top")
        )
    ...
    it("check simple datatype") {
      ...
      val size = udf { x: Seq[Row] => x.size }
      val limited = df.agg(CollectLimit.collect_truncate($"num", 4).as("lst"))
    ...
    it("incomplete buckets") {
      ...
      val limited = df.
        groupBy($"id").
        agg(
          CollectLimit.collect_truncate($"name", 2).as("names")
        )
    ...
    it("null test") {
      ...
      val limited = df.agg(CollectLimit.collect_truncate($"num", 2).as("nums"))
